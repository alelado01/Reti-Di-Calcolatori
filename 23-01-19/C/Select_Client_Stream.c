...
{
  int sd, nread, port, size;
  char c, ok, nome_file[LENGTH_FILE_NAME], targa[LENGTH], filename[MAX_LENGTH_FILENAME];
  struct hostent *host;
  ...
  while (gets(targa))
  {
    ...
    if (ok == 'S')
    {
      printf("Ricevo il nome del file:\n");
      if (read(sd, &filename, MAX_LENGTH_FILENAME) < 0)
      {
        perror("read name file");
        break;
      }
      printf("Nome del file ricevuto: %s\n\n", filename);
      ...
      printf("Ricevo l'immagine:\n");
      if (read(sd, &receive_buff, sizeof(receive_buff)) < 0)
      {
        perror("read");
        break;
      }
      printf("Immagine %s ricevuta con successo!\n\n", filename);
    }
    else if (ok == 'N')
