...
#define PAROLA_LENGTH 13

/*** STRUTTURA DA INVIARE ATTRAVERO LA SOCKET ***/
// struttura di esempio:
typedef struct{
	char nome[MAX_FILE_LENGTH];
	char parola[PAROLA_LENGTH];
} Request;
...
int main(int argc, char **argv)
{
	...
	Request req;
	char filename[MAX_FILE_LENGTH], parola[PAROLA_LENGTH];
  ...
  * host=gethostbyname("localhost");
  ...
  if( clientaddr.sin_addr.s_addr == INADDR_NONE )
  {
	...
	{	
		...
		if (ris >= 0)
		{
			printf("Parola presente nel file: %d\n", ris);
		}
		else
			printf("Errore, parola non trovata\n");
