...
int main(int argc, char **argv)
{
	...
	char buff[DIM_BUFF], line[DIM_BUFF], parola[WORD_LENGTH], subString[WORD_LENGTH], tmp_file[MAX_LENGTH_FILENAME], filename[MAX_LENGTH_FILENAME], path[1000], risp; // tipi di risposte
	int qualcosa;
	...
	for (;;)
	{
		...
		if (FD_ISSET(udpfd, &rset))
		{
			...
			if (recvfrom(udpfd, &parola, sizeof(parola), 0, (struct sockaddr *)&clientaddr, &len) < 0)
			{
				perror("Recvfrom");
				continue;
			}
			printf("Richiesto dal client questa parola: %s\n", parola);
			esito = -1;
